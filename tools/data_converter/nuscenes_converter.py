...
from nuscenes.nuscenes import NuScenes
nusc_trainval = NuScenes(version="v1.0-trainval", dataroot="data/nuscenes", verbose=True)

def build_scenewise_keyframe_index(nusc_trainval):
    scenewise_ts = defaultdict(list)
    scenewise_tokens = defaultdict(list)
    for s in nusc_trainval.sample:
        scene_token = s['scene_token']
        scenewise_ts[scene_token].append(s['timestamp'])
        scenewise_tokens[scene_token].append(s['token'])
    return scenewise_ts, scenewise_tokens
# build keyframe (2Hz_train_val) timestamps and tokens
scenewise_ts, scenewise_tokens = build_scenewise_keyframe_index(nusc_trainval)
...
def create_nuscenes_infos(
    ...
    scenes_token_name = {
        available_scenes[available_scene_names.index(s)]["token"]: s
    ...
    train_nusc_infos, train_scene_tokens, val_nusc_infos, val_scene_tokens = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, scenes_token_name, test, max_sweeps=max_sweeps,
...
def find_nearest_keyframe_in_scene(ts, timestamps, tokens):
    ...
    if abs(ts - before) <= abs(ts - after):
        return tokens[index - 1]
    else:
        return tokens[index]

def get_cam_front_path(nusc, sample):
    cam_token = sample['data']['CAM_FRONT']
    sd = nusc.get('sample_data', cam_token)
    return os.path.join(nusc.dataroot, sd['filename'])

def _sample_data_transform(nusc, sample, max_sweeps, scenes_token_name, 
                        ego_pose_token=None, cam_tokens=None, keyframe=True):
    ...
    # find nearest_keyframe token
    scene_token = sample['scene_token']
    ts = sample['timestamp']
    nearest_keyframe_token = find_nearest_keyframe_in_scene(
        ts, scenewise_ts[scene_token], scenewise_tokens[scene_token]
    )
    # # print CAM_FRONT path
    # cam_front_interp = get_cam_front_path(nusc, sample)
    # keyframe_sample = nusc_trainval.get('sample', nearest_keyframe_token)
    # cam_front_keyframe = get_cam_front_path(nusc_trainval, keyframe_sample)
    # print(f"Interp Sample: {sample['token']}")
    # print(f"  ↳ CAM_FRONT: {cam_front_interp}")
    # print(f"Nearest Keyframe: {nearest_keyframe_token}")
    # print(f"  ↳ CAM_FRONT: {cam_front_keyframe}")
    # print("-" * 60)

    info = {
        "scene_name": scenes_token_name[sample['scene_token']],
        "scene_token": sample["scene_token"],
        "token": sample["token"],
        ...
        "nearest_keyframe_token": nearest_keyframe_token,
    }
...
def _fill_trainval_infos(
        nusc, train_scenes, val_scenes, scenes_token_name, 
        test=False, max_sweeps=10, with_cam_sweeps=True):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        info = _sample_data_transform(
            nusc, sample, max_sweeps=max_sweeps, scenes_token_name=scenes_token_name, keyframe=True)
        ...
        if with_cam_sweeps:
            ...
            while cams is not None and cams[0]['sample_token'] == sample['token']:
                ...
                info = _sample_data_transform(
                    ...
                    scenes_token_name=scenes_token_name,
