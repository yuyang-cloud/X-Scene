...
sys.path.append(".")
from xscene.dataset.pipeline_utils import one_hot_decode, one_hot_encode
...
@hydra.main(version_base=None, config_path="../configs", config_name="config_video")
def main(cfg: DictConfig):
    ...
    cfg.dataset.train_pipeline[9].safe = False  # ReorderMultiViewImagesM
    cfg.dataset.test_pipeline[-1]["keys"].append("gt_masks_bev_static")
    cfg.dataset.test_pipeline[-1].meta_lis_keys.append("token")
    cfg.dataset.test_pipeline[8].safe = False

    # set start_on_keyframe and temporal to False to save each frame 12Hz(keyframe + sweeps)
    cfg.dataset.data.train.start_on_keyframe = False
    cfg.dataset.data.val.start_on_keyframe = False
    cfg.dataset.data.test.start_on_keyframe = False
    cfg.dataset.data.train.temporal = False
    cfg.dataset.data.val.temporal = False
    cfg.dataset.data.test.temporal = False
    ...
    output_pth = "data/nuscenes/nuscenes_map_aux_12Hz_interp"
    os.makedirs(output_pth, exist_ok=True)
    with h5py.File(f"{output_pth}/{cfg.process}_200x200_{cfg.subfix}.h5", "w") as h5:
