...
def check_scene_complete(token: str) -> bool:
    for gen_id in range(4):  # gen0, gen1, gen2, gen3
        for cam in CAM_NAMES:
            filename = f"{token}_{cam}.mp4"
            path = os.path.join(cfg.log_root, f"{token}_gen{gen_id}", filename)
...
def shard_by_rank(seq, rank, world_size, mode="stride"):
    n = len(seq)
...
def generate_scene(val_dataloader, scene, overlaps, generator=None):
    ...
    progress_bar = tqdm(range(len(val_dataloader) * cfg.runner.validation_times),
                        desc="Steps", disable=not is_main)
    ...
    for val_input in val_dataloader:
        ...
        if scene_type == 'night':
            val_input['captions_clip'] = ['A driving scene image at singapore-hollandvillage. night, clear, suburban, streetlights.']*len(val_input['captions_clip'])
            val_input['captions_t5'] = ['A driving scene image at singapore-hollandvillage. night, clear, suburban, streetlights.']*len(val_input['captions_t5'])
        elif scene_type == 'rainy':
            val_input['captions_clip'] = ['A driving scene image at boston-seaport. rainy, cloudy, suburban, wet road.']*len(val_input['captions_clip'])
            val_input['captions_t5'] = ['A driving scene image at boston-seaport. rainy, cloudy, suburban, wet road.']*len(val_input['captions_t5'])
        ###
        ...
        for _, ori_imgs, _, _, gen_imgs_list, _, _ in zip(*return_tuples):
            # # save ori
            if save_ori:
    ...
    if mode == 'standarded':
        ...
        for scene in scene_tokens:
            for start in start_idxs:
                ...
                if start > 0:
                    sample_pool = range(max(start - candidate_length, 0), start)
                    k = min(len(sample_pool), ref_length)
                    ref_idx = sorted(random.sample(sample_pool, k))
                    if k < ref_length:
                        ref_idx = [ref_idx[0]] * (ref_length - k) + ref_idx
                else: # start == 0
                    ref_idx = [0] * ref_length
                ref = [token_data_dict[scene[idx]] for idx in ref_idx]
                clip = ref + clip
                scene_clips[scene[0]].append(clip)

        # distribute scenes to different ranks
        print(len(scene_clips), "scenes in total.")
        items = sorted(scene_clips.items(), key=lambda x: x[0])
        items = shard_by_rank(items, rank, world_size, mode="stride")
        print(len(items), f"scenes for rank {rank}.")

        for i, (k, v) in enumerate(items):
            if check_scene_complete(k):
                print(f"[rank{rank}]  Scene {k} already completed, skipping.")
                continue
            print(f"[rank{rank}] Processing scene: {k}, {len(v)} clips")
        ...
        # distribute scenes to different ranks
        scene_tokens = shard_by_rank(scene_tokens, rank, world_size, mode="stride")
...
def _get_args():
    ...
    parser.add_argument("-r", "--resize", type=str, default='bilinear')
    parser.add_argument("-i", "--input", type=str, default='data/nuscenes/workshop/nuscenes_interp_12Hz_infos_track2_eval.pkl')
    parser.add_argument("-o", "--output", type=str, default='./work_dirs/test_video_submit')
...
if __name__ == '__main__':
    ...
    args = _get_args()
    pipe_single, cfg_single, _ = load_pipe(args.model_img, config_name="test_config_img", device=device)
    pipe, cfg, weight_dtype = load_pipe(args.model_video, config_name="test_config_video", device=device)
