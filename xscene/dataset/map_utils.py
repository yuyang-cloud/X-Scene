...
from shapely.geometry import Polygon, MultiPolygon, LineString, MultiLineString,Point, box, MultiPoint
...
from xscene.runner.box_visualizer import box_center_shift
...
def visualize_bev_hdmap(gt_lines_instance, gt_labels_3d, canvas_size, bound=[-50.0, 50.0], vis_format='polyline_pts', drivable_mask=None):
...
def project_map_to_image(gt_lines_instance, gt_labels_3d, intrinsic, extrinsic, image=None, drivable_mask=None, image_size=[224, 400]):
    z = 0
    ...
    for gt_line_instance, gt_label_3d in zip(gt_lines_instance, gt_labels_3d):
        ...
        points_in_cam_cor = points_in_cam_cor[:, points_in_cam_cor[2, :] > 1e-5]
    ...
    if drivable_mask is not None:
        ...
        if points_in_cam_cor.shape[1] > 1:
            ...
            if image is not None:
                for point in points_on_image_cor:
                    cv2.circle(image, point, 20, (100,0,0), -1)

    if image is not None:
        cv2.resize(image, (400, 224))
        cv2.imwrite('./layout.png', image)
...
def project_box_to_image(gt_bboxes_3d, gt_labels_3d, transform, object_classes, image=None, cam_name=None, image_size=[224, 400]):
    
    bboxes_trans = box_center_shift(gt_bboxes_3d, (0.5, 0.5, 0.5))
    if image is not None:
        image = visualize_camera(
                './layout_box.png', image=image, bboxes=bboxes_trans, labels=gt_labels_3d,
                transform=transform,classes=object_classes, thickness=1
        )

    canvas = np.zeros((len(object_classes), 900, 1600, 3), dtype=np.uint8)

    if bboxes_trans is not None and len(bboxes_trans) > 0:
...
class NuScenesMap:
    ...
    def render_map_in_image(self,
        ...
        return self.explorer.render_map_in_image(
            ...
            layer_names=layer_names, verbose=verbose, 
            input_image=input_image, out_path=out_path)
...
class NuScenesMapExplorer:
    ...
    def render_layers(self,
        ...
        # fig = plt.figure(figsize=self._get_figsize(figsize))
        # ax = fig.add_axes([0, 0, self.canvas_aspect_ratio, 1])
        fig, ax = plt.subplots(figsize=self._get_figsize(figsize))
        # import pdb; pdb.set_trace()
    ...
    def _get_figsize(self, figsize: Union[None, float, Tuple[float, float]]) -> Tuple[float, float]:
        ...
        canvas_size = np.array(self.map_api.canvas_edge)[::] / 200
