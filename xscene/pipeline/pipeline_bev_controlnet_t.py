...
def roll_latents(latents, view_order, roll_length=None):
    """
    Roll latents for panoramic generation to ensure seamless connection between views.
    This is particularly useful for 360-degree scenes.

    Args:
        latents (torch.Tensor): The latents for all camera views. Shape: (n_cam, c, h, w)
        view_order (List[str]): The order of camera views.
        roll_length (int, optional): The number of pixels to roll. Defaults to w // 2.
    Returns:
        torch.Tensor: The rolled latents.
    """
...
class StableDiffusionBEVControlNetTPipeline(StableDiffusionBEVControlNetPipeline):
    def __init__(
        self,
        vae: AutoencoderKL,
    ...
    def __call__(
        ...
        if do_classifier_free_guidance and not guess_mode:
            ...
            image = torch.cat(_images)
            layout_canvas = torch.concat([torch.zeros_like(layout_canvas).to(device=self.device), layout_canvas], dim=0) if layout_canvas is not None else None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if double_cfg_inference and do_classifier_free_guidance:
                    ...
                    latent_model_input = latent_model_input.chunk(2)
                    encoder_hidden_states_with_cam = encoder_hidden_states_with_cam.chunk(2)
                    encoder_hidden_states_with_cam_t5 = encoder_hidden_states_with_cam_t5.chunk(2)
                    mid_block_res_sample = mid_block_res_sample.chunk(2)
                    _down_block_res_samples = [None, None]
                    _down_block_res_samples[0] = [
                        _downs.chunk(2)[0] for _downs in down_block_res_samples]
                    _down_block_res_samples[1] = [
                        _downs.chunk(2)[1] for _downs in down_block_res_samples]
                    ...
                    unet_out_uncond = self.unet(
                        ...
                        encoder_hidden_states_t5=encoder_hidden_states_with_cam_t5[0],
                    ...
                    unet_out_cond = self.unet(
                        ...
                        encoder_hidden_states_t5=encoder_hidden_states_with_cam_t5[1],
                        # if use original unet, it cannot take kwargs
                        cross_attention_kwargs=cross_attention_kwargs,
