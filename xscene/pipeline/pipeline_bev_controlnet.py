...
class StableDiffusionBEVControlNetPipeline(StableDiffusionControlNetPipeline):
    def __init__(
        ...
        text_encoder_t5: T5EncoderModel = None,
    ...
    def _encode_prompt_t5(
        ...
        if prompt_embeds is None:
            ...
            if isinstance(self, TextualInversionLoaderMixin):
                prompt = self.maybe_convert_prompt(prompt, self.tokenizer_t5)
            ...
            text_input_ids = self.tokenizer_t5.pad(
                {"input_ids": input_ids}, padding=True, return_tensors="pt"
            ).input_ids

            untruncated_ids = self.tokenizer_t5(prompt, padding="longest", return_tensors="pt").input_ids

            if untruncated_ids.shape[-1] >= text_input_ids.shape[-1] and not torch.equal(
                text_input_ids, untruncated_ids
            ):
                removed_text = self.tokenizer_t5.batch_decode(
                    untruncated_ids[:, self.tokenizer_t5.model_max_length - 1 : -1]
                )
                logger.warning(
                    "The following part of your input was truncated because T5 can only handle sequences up to"
                    f" {self.tokenizer_t5.model_max_length} tokens: {removed_text}"
                )

            if hasattr(self.text_encoder_t5.config, "use_attention_mask") and self.text_encoder_t5.config.use_attention_mask:
                attention_mask = text_inputs.attention_mask.to(device)
            else:
                attention_mask = None

            prompt_embeds = self.text_encoder_t5(
                text_input_ids.to(device),
                attention_mask=attention_mask,
            )
            prompt_embeds = prompt_embeds[0]

        prompt_embeds = prompt_embeds.to(dtype=self.text_encoder_t5.dtype, device=device)
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            # textual inversion: procecss multi-vector tokens if necessary
            if isinstance(self, TextualInversionLoaderMixin):
                uncond_tokens = self.maybe_convert_prompt(uncond_tokens, self.tokenizer_t5)

            max_length = prompt_embeds.shape[1]
            uncond_input = self.tokenizer_t5(
                uncond_tokens,
                padding="max_length",
            ...
            negative_prompt_embeds = self.text_encoder_t5(
        ...
        if do_classifier_free_guidance:
            ...
            negative_prompt_embeds = negative_prompt_embeds.to(dtype=self.text_encoder_t5.dtype, device=device)
    ...
    def __call__(
        ...
        prompt_embeds_t5 = self._encode_prompt_t5(
            prompt_t5,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        if len(image) == 5:
            _, image, layout_canvas, occ_render_img, occ_render_depth = image[:5]
            layout_canvas = move_to(layout_canvas, self.device)
            occ_render_img = move_to(occ_render_img, self.device)
            occ_render_depth = move_to(occ_render_depth, self.device)
        elif len(image) == 3:
            _, image, layout_canvas = image[:3]
            layout_canvas = move_to(layout_canvas, self.device)
        ...
        if do_classifier_free_guidance and not guess_mode:
            ...
            occ_render_img = torch.concat([torch.zeros_like(occ_render_img).to(device=self.device), occ_render_img], dim=0) if occ_render_img is not None else None
            occ_render_depth = torch.concat([torch.zeros_like(occ_render_depth).to(device=self.device), occ_render_depth], dim=0) if occ_render_depth is not None else None
            bev_controlnet_kwargs = move_to(kwargs_with_uncond, self.device)
        ###### BEV end ######

        # scene positional embedding
        if self.scene_embedder is not None:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if guess_mode and do_classifier_free_guidance:
                    ...
                    controlnet_prompt_embeds_t5 = prompt_embeds_t5.chunk(2)[1]
                    if self.scene_embedder is not None:
                        controlnet_scene_embedding = scene_embedding
                    else:
                        controlnet_scene_embedding = None
                else:
                    controlnet_latent_model_input = latent_model_input
                    controlnet_prompt_embeds = prompt_embeds
                    controlnet_prompt_embeds_t5 = prompt_embeds_t5
                    if self.scene_embedder is not None:
                ...
                encoder_hidden_states_with_cam, encoder_hidden_states_with_cam_t5 = self.controlnet(
                    controlnet_latent_model_input,
                    controlnet_t,
                    camera_param,  # for BEV
                    encoder_hidden_states=controlnet_prompt_embeds,
                    encoder_hidden_states_t5=controlnet_prompt_embeds_t5,
                    controlnet_cond=[_, image, layout_canvas, occ_render_img, occ_render_depth],
                ...
                if guess_mode and do_classifier_free_guidance:
                    ...
                    # add uncond encoder_hidden_states_with_cam here
                    encoder_hidden_states_with_cam_t5 = self.controlnet.add_uncond_to_emb(
                        prompt_embeds_t5.chunk(2)[0], N_cam,
                        encoder_hidden_states_with_cam_t5,
                    )
                ...
                # scene positional embedding
                if self.scene_embedder is not None and self.reference_control_reader is not None:
                    ...
                    self.reference_control_reader.update([latent_scene_embedding])

                noise_pred = self.unet(
                    latent_model_input,  # may with unconditional
                    t,
                    encoder_hidden_states=encoder_hidden_states_with_cam,
                    encoder_hidden_states_t5=encoder_hidden_states_with_cam_t5,
