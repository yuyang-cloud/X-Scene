...
from xscene.dataset.utils import collate_t_fn
...
from xscene.networks.unet_2d_condition_multiview import UNet2DConditionModelMultiview
...
class ControlnetUnetEmbedderWrapper(ModelMixin):
    ...
    def forward(self, noisy_latents, noisy_latents_unet, scene_embedding,
                can_bus, timesteps, timesteps_unet, camera_param, 
                encoder_hidden_states, encoder_hidden_states_uncond, 
                encoder_hidden_states_t5, encoder_hidden_states_uncond_t5,
                controlnet_image, **batch_kwargs):
        N_cam = noisy_latents.shape[1]

        # fmt: off
        down_block_res_samples, mid_block_res_sample, \
        encoder_hidden_states_with_cam, encoder_hidden_states_t5_with_cam = self.controlnet(
            ...
            encoder_hidden_states_uncond_t5=encoder_hidden_states_uncond_t5,
        ...
        with context(**context_kwargs):
            ...
            model_pred = self.unet(
                ...
                timesteps_unet.reshape(-1),  # b x n
                encoder_hidden_states=encoder_hidden_states_with_cam.to(
                    dtype=self.weight_dtype
                ),  # b x n, len + 1, 768
                encoder_hidden_states_t5=encoder_hidden_states_t5_with_cam.to(
                    dtype=self.weight_dtype
                ),
...
class MultiviewTRunner(MultiviewRunner):
    def __init__(self, cfg, accelerator, train_set, val_set) -> None:
        ...
        self.validator = BaseTValidator(
            ...
            pipe_param={
                ...
                "text_encoder_t5": self.text_encoder_t5,
                "tokenizer": self.tokenizer,
                "tokenizer_t5": self.tokenizer_t5,
            }
        )
    ...
    def _set_dataset_loader(self):
        # dataset
        collate_fn_param = {
            "tokenizer_clip": self.tokenizer if hasattr(self, "tokenizer") else None,
            "tokenizer_t5": self.tokenizer_t5 if hasattr(self, "tokenizer_t5") else None,
            "template_clip": self.cfg.dataset.template_clip,
            "template_t5": self.cfg.dataset.template_t5,
        ...
        if self.train_dataset is not None:
            self.train_dataloader = torch.utils.data.DataLoader(
                ...
                collate_fn=partial(
                    collate_t_fn, is_train=True, **collate_fn_param),
        ...
        if self.val_dataset is not None:
            self.val_dataloader = torch.utils.data.DataLoader(
                ...
                collate_fn=partial(
                    collate_t_fn, is_train=False, **collate_fn_param),
    ...
    def _init_fixed_models(self, cfg):
        ...
        self.text_encoder_t5 = T5EncoderModel.from_pretrained(cfg.model.pretrained_t5_path)
    ...
    def _init_trainable_models(self, cfg):
        assert cfg.model.pretrained_img_unet is not None
        ...
        if cfg.model.fix_controlnet:
            unet = UNet2DConditionModelMultiview.from_pretrained(
                cfg.model.pretrained_img_unet, subfolder="unet")
            self.unet = model_cls.from_unet_2d_condition(unet, **unet_param)
        else:
            self.unet = model_cls.from_pretrained(
                cfg.model.pretrained_img_unet, subfolder="unet", **unet_param)    
        ...
        controlnet = model_cls.from_pretrained(
            cfg.model.pretrained_img_unet, subfolder=cfg.model.controlnet_dir)
        ...
        state_dict = torch.load(os.path.join(cfg.model.pretrained_img_unet, cfg.model.scene_embedder_dir, "scene_embedder_model.bin"), map_location='cpu')
        self.scene_embedder.load_param(state_dict)
        logging.info(
                f"[MultiviewTRunner] load weight of scene_embedder")
        ...
        if not cfg.model.fix_controlnet:
            self.accelerator.wait_for_everyone()
            state_dict = torch.load(os.path.join(cfg.model.pretrained_img_unet, cfg.model.can_bus_embedder_dir, "can_bus_embedder_model.bin"), map_location='cpu')
            self.can_bus_embedder.load_state_dict(state_dict)
            logging.info(
                    f"[MultiviewTRunner] load weight of can_bus_embedder")
    ...
    def _set_model_trainable_state(self, train=True):
        ...
        if self.cfg.model.fix_controlnet:
            ...
            for name, mod in self.scene_embedder.named_parameters():
                logging.debug(
                    f"[MultiviewTRunner] set {name} to requires_grad = True")
    ...
    def _train_one_stop(self, batch):
        ...
        with self.accelerator.accumulate(self.controlnet_unet_embedder):
            ...
            #### here we change (b, l, ...) to (bxl, ...) ####
            # Get the text embedding for conditioning using clip
            encoder_hidden_states = self.text_encoder(
                f_to_b(batch["input_ids_clip"]))[0]
            encoder_hidden_states_uncond = self.text_encoder(
                f_to_b(batch["uncond_ids_clip"]))[0]
            # Get the text embedding for conditioning using t5
            encoder_hidden_states_t5 = self.text_encoder_t5(
                f_to_b(batch["input_ids_t5"]))[0]
            encoder_hidden_states_uncond_t5 = self.text_encoder_t5(
                f_to_b(batch["uncond_ids_t5"]))[0]


            controlnet_image = f_to_b(batch["bev_map_with_aux"].to(
                dtype=self.weight_dtype))
            controlnet_image = [controlnet_image]
            # layout canvas
            if 'with_layout_canvas' in self.cfg.model.controlnet and self.cfg.model.controlnet.with_layout_canvas:
                bev_hdmap = f_to_b(batch["bev_hdmap"].to(dtype=self.weight_dtype))
                layout_canvas = f_to_b(batch["layout_canvas"].to(dtype=self.weight_dtype))
                controlnet_image += [bev_hdmap, layout_canvas]
            # occ render semantic_image and depth
            if 'with_occ_render_img' in self.cfg.model.controlnet and self.cfg.model.controlnet.with_occ_render_img:
                occ_render_img = f_to_b(batch["occ_render_image"].to(dtype=self.weight_dtype))
                occ_render_depth = f_to_b(batch["occ_render_depth"].to(dtype=self.weight_dtype))
                controlnet_image += [occ_render_img, occ_render_depth]

            model_pred = self.controlnet_unet_embedder(
                noisy_latents, noisy_latents_unet, scene_embedding,
                can_bus, timesteps, timesteps_unet, camera_param, 
                encoder_hidden_states, encoder_hidden_states_uncond, 
                encoder_hidden_states_t5, encoder_hidden_states_uncond_t5,
                controlnet_image, **batch_kwargs,
