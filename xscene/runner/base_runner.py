...
from xscene.misc.common import (
    move_to,
    load_module,
...
class BaseRunner:
    ...
    def _set_dataset_loader(self):
        ...
        collate_fn_param = {
            ...
            "map_cond_type": self.cfg.model.get('map_cond_type', 'bev_seg'),
        }
    ...
    def run(self):
        ...
        if self.cfg.resume_from_checkpoint:
            ...
            if path is None:
            ...
            elif 'resume_only_model' in self.cfg and self.cfg.resume_only_model:
                self.accelerator.print(
                    f"Resuming model from checkpoint {self.cfg.resume_from_checkpoint}"
                )
                ...
                for i, model in enumerate(self.accelerator._models):
                    weights_name = f"pytorch_model.bin" if i == 0 else f"pytorch_model_{i}.bin"
                    input_model_file = os.path.join(load_path, weights_name)
                    checkpoint = torch.load(input_model_file, map_location='cpu')
                    missing_keys, unexpected_keys = self.accelerator._models[i].load_state_dict(checkpoint, strict=False)
                    del checkpoint
                logging.info(
                    f"[BaseRunner] load pretrained model with "
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if global_step % self.cfg.runner.checkpointing_steps == 0:
                        ...
                        #     self._cleanup_checkpoints(self.cfg.log_root)
                        # self.accelerator.wait_for_everyone()
    ...
    def _cleanup_checkpoints(self, log_root, num_keep=1):
        checkpoint_dirs = sorted(glob.glob(os.path.join(log_root, "checkpoint-*")), key=os.path.getmtime)
        
        # delet old checkpoints to save disk space
        while len(checkpoint_dirs) > num_keep:
            oldest = checkpoint_dirs.pop(0)
            logging.info(f"Deleting old checkpoint directory: {oldest}")
