...
from .img_utils import visualize_image_with_canvas
...
class MultiviewRunner(BaseRunner):
    ...
    def _init_trainable_models(self, cfg):
        ...
        self.unet = model_cls.from_unet_2d_condition(unet, **unet_param)    # new parm in cross-view attention
        ...
        self.controlnet = model_cls.from_unet(unet, load_param=True, **controlnet_param)
    ...
    def set_optimizer_scheduler(self):
        ...
        param_count = smart_param_count(params_to_optimize)
        logging.info(
            f"[MultiviewRunner] have total {param_count} params from unet and controlnet to optimizer.")
        if hasattr(self.cfg.model, "scene_embedder_cls"):
    ...
    def prepare_device(self):
        if hasattr(self.cfg.model, "scene_embedder_cls"):
            ...
            self.controlnet_unet = ControlnetUnetWrapper(self.controlnet, self.unet, self.scene_embedder, self.reference_control_reader)
        else:
    ...
    def _train_one_stop(self, batch):
        ...
        with self.accelerator.accumulate(self.controlnet_unet):
            ...
            encoder_hidden_states = self.text_encoder(batch["input_ids_clip"])[0]
            encoder_hidden_states_uncond = self.text_encoder(
                batch["uncond_ids_clip"])[0]
            # Get the text embedding for conditioning using t5
            encoder_hidden_states_t5 = self.text_encoder_t5(batch["input_ids_t5"])[0]
            encoder_hidden_states_uncond_t5 = self.text_encoder_t5(
                batch["uncond_ids_t5"])[0]
            ...
            if 'with_occ_render_img' in self.cfg.model.controlnet and self.cfg.model.controlnet.with_occ_render_img:
                occ_render_img = batch["occ_render_image"].to(dtype=self.weight_dtype)
                occ_render_depth = batch["occ_render_depth"].to(dtype=self.weight_dtype)
            ...
            model_pred = self.controlnet_unet(
                ...
                encoder_hidden_states, encoder_hidden_states_uncond, 
                encoder_hidden_states_t5, encoder_hidden_states_uncond_t5,
                controlnet_image, scene_embedding=scene_embedding,
                **batch['kwargs'],
            )
