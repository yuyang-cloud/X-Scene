...
from .blocks import (
    LongShortTemporalMultiviewTransformerBlock,
    Transformer2DModelT5,
    CrossAttnDownBlock2DT5,
    CrossAttnUpBlock2DT5,
    UNetMidBlock2DCrossAttnT5,
)


class UNet2DConditionModelMultiviewT(UNet2DConditionModel):
    r"""
    ...
    def __init__(
        ...
        for name, mod in list(self.named_modules()):
            if isinstance(mod, CrossAttnDownBlock2D):
            ...
            elif isinstance(mod, CrossAttnUpBlock2D):
                _set_module(self, name, CrossAttnUpBlock2DT5(**mod._args,))
                ...
                _set_module(self, name, Transformer2DModelT5(**mod._args,
                                                             ...
                                                             attn1_q_trainable=attn1_q_trainable,
                                                             temporal_attention=True,)) # NOTE: we set temporal_attention=True here
        # set the new module to be trainable: T5 cross-attn and multiview cross-attn
        assert crossview_attn_type == "temporal_t5_crossview"
        for name, mod in list(self.named_modules()):
            if isinstance(mod, BasicTransformerBlock):
                _set_module(self, name, LongShortTemporalMultiviewTransformerBlock(
                    **mod._args,
                    # multi_view
        ...
        self.n_cam = len(neighboring_view_pair)
        ...
        for module in self.modules():
            if isinstance(module, (CrossAttnDownBlock2D, DownBlock2D, CrossAttnUpBlock2D, UpBlock2D)):
                ...
                if flag is not None and not flag[mod_idx]:
                    logging.debug(
                        f"[UNet2DConditionModelMultiviewT] "
                        f"gradient_checkpointing skip [{module.__class__}]")
                    continue
                logging.debug(f"[UNet2DConditionModelMultiviewT] set "
    ...
    def forward(
            ...
            for down_block_res_sample, down_block_additional_residual in zip(
                ...
                if self.with_ref:
                    down_block_res_sample = rearrange(
                        down_block_res_sample, "(b f n) ... -> b f n ...",
                        f=self.ref_length+self.video_length, n=self.n_cam)
                    down_block_additional_residual = rearrange(
                        down_block_additional_residual, "(b f n) ... -> b f n ...",
                        f=self.video_length, n=self.n_cam)
                    down_block_res_sample_ori = down_block_res_sample[:, self.ref_length:] + down_block_additional_residual
                    down_block_res_sample = torch.cat([down_block_res_sample[:, :self.ref_length], down_block_res_sample_ori], dim=1)
                    down_block_res_sample = rearrange(
                        down_block_res_sample, "b f n ... -> (b f n) ...")
                else:
        ...
        if mid_block_additional_residual is not None:
            if self.with_ref:
                ...
                mid_block_additional_residual = rearrange(
                    mid_block_additional_residual, "(b f n) ... -> b f n ...",
                    f=self.video_length, n=self.n_cam)
                sample_ori = sample[:, self.ref_length:] + mid_block_additional_residual
                sample = torch.cat([sample[:, :self.ref_length], sample_ori], dim=1)
                sample = rearrange(
                    sample, "b f n ... -> (b f n) ...")
            else:
        ...
        if self.with_ref:
            sample = rearrange(
                sample, "(b f n) ... -> b f n ...",
                f=self.ref_length+self.video_length, n=self.n_cam)
            sample = sample[:, self.ref_length:].clone()
            sample = rearrange(
                sample, "b f n ... -> (b f n) ...")
