...
class ScenePositionEmbedding(nn.Module):
    """
    ...
    def __init__(
        ...
        self.conv_out = zero_module(
            nn.Conv2d(block_out_channels[-1], embed_dims, kernel_size=3, padding=1)
        )
    ...
    def forward(self, img_feats, img_metas):
        ...
        if len(img_feats.shape) == 6:
            assert img_feats.shape[0] == 1
            B, N = img_feats.shape[1:3]
        else:
        ...
        img2lidars = []
        for lidar2image in img_metas['lidar2image'] if len(img_feats.shape) == 5 else img_metas['lidar2image'][0]:
            img2lidar = []
        ...
        # get box mask
        object_masks = []
        for bi, (gt_bboxes_3d, points) in enumerate(zip(img_metas['gt_bboxes_3d'] if len(img_feats.shape) == 5 else img_metas['gt_bboxes_3d'][0], coords3d)):
            ...
            mask = torch.cat([points_in_boxes_all(points[i].unsqueeze(0), boxes.unsqueeze(0)).any(-1).squeeze(0) for i in range(chunk_num)])
