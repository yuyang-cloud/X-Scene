...
class LoraInjectedLinear(nn.Module):
    def __init__(
        self, in_features, out_features, bias=False, r=4, dropout_p=0.0, scale=1.0
    ):
        super().__init__()
...
TEXT_ENCODER_DEFAULT_TARGET_REPLACE = {"CLIPAttention", "T5Attention"}

TEXT_ENCODER_EXTENDED_TARGET_REPLACE = {"CLIPAttention", "T5Attention"}
...
def _find_modules_v2(
    ...
    #             # Otherwise, yield it
    #             yield parent, name, module
    for ancestor_name, ancestor in ancestors.items():
        for fullname, module in ancestor.named_modules():
            if any(isinstance(module, _class) for _class in search_class):
                *path, name = fullname.split(".")
                parent = ancestor
                ...
                if exclude_children_of and any(
                    ...
                    continue
                # Otherwise, yield it
                yield ancestor_name, parent, name, module
...
def inject_trainable_lora(
    ...
    for _module_name, _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class=[nn.Linear]
    ):
        # ignore multi-view attention
        if 'attn4' in _module_name:
            continue
...
def inject_trainable_lora_extended(
    ...
    for _module_name, _module, name, _child_module in _find_modules(
        ...
        # Linear/Conv2d eval
        if isinstance(_tmp, LoraInjectedLinear):
            _module._modules[name].linear.eval()
        elif isinstance(_tmp, LoraInjectedConv2d):
            _module._modules[name].conv.eval()

        # lora_up, lora_down train
        if train:
            _module._modules[name].lora_up.train()
            _module._modules[name].lora_down.train()
        else:
            _module._modules[name].lora_up.eval()
            _module._modules[name].lora_down.eval()
...
def extract_lora_ups_down(model, target_replace_module=DEFAULT_TARGET_REPLACE):
    ...
    for _module_name, _m, _n, _child_module in _find_modules(
        model,
...
def collapse_lora(model, target_replace_module=DEFAULT_TARGET_REPLACE, alpha=1.0):
    log_list_lin, log_list_conv = [], []
    for _module_name, _module, name, _child_module in _find_modules(
        model,
        target_replace_module,  #UNET_EXTENDED_TARGET_REPLACE | TEXT_ENCODER_EXTENDED_TARGET_REPLACE,
        search_class=[LoraInjectedLinear, LoraInjectedConv2d],
    ):

        if isinstance(_child_module, LoraInjectedLinear):
            # print("Collapsing Lin Lora in", name)
            log_list_lin.append(name)
    ...
    # print("Collapsed Lin Lora in", log_list_lin)
    # print("Collapsed Conv Lora in", log_list_conv)
